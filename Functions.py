...
def KM_Coeff_1(data,dim= 1,time_res = 1,bandwidth=0.1,dist = 500, order = 3, start_value=0):
  if dim == 1:
    powers = [0,1,2]
    bins = np.array([6000])
    kmc,edges = km(data,powers = powers,bins = bins,bw=bandwidth)
...
def power_mismatch(data,avg_for_each_hour = True, time_res = 1,dispatch=2,start_minute=0,end_minute=7,length_seconds_of_interval=5):
    ...
    end = 2*length_seconds_of_interval
    steps = end +1
    ...
    daily_prof_25[24*3600*time_res:] = day[0:1*24*3600*time_res]
...
def Euler_Maruyama(data,c_1,c_2,Delta_P,epsilon,c_1_weight = 3,time_res = 1,dispatch = 1,delta_t=0.1,t_final=5,model=3,factor_daily_profile=0):
    ...
    for i in range(1,time.size):
      ...
      omega[i] = omega[i-1] + delta_t * (  1 * c_1_weight[i-1] * c_1_fun(omega[i-1]) - 1 * c_2_fun(omega[i-1]) * theta[i-1] + 1*Delta_P_fun(i) )  + 1*epsilon_fun(omega[i-1])*dW[i]
    if model == 4:
